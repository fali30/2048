//
//  2048
//     Text and graphical version of the game 1024
...
void displayAsciiBoard( int board[], int squaresPerSide, int score, int moveNumber, Node *pHead)
{
    std::cout << "\n"
...
//--------------------------------------------------------------------
// Display the Graphical Board
// void displayGraphicalBoard( int board[], int squaresPerSide, int score) {
...
{	
    ...
    for ( int i = 0; i < squaresPerSide*squaresPerSide; ++i) {
        pHead->boardData[i] = board[i];
    }

    
    pHead->scoreData = score;
    pHead->moveData = moveNumber;
    pHead->pNext = NULL;
	...
	while (window.isOpen())
	{
        ...
        displayAsciiBoard( board, squaresPerSide, score, moveNumber, pHead);
        ...
        int arraySize = squaresPerSide * squaresPerSide;
        int squareSize = 400 / squaresPerSide - 10;
        Square squaresArray[ arraySize + 1];
        for( int row = 0; row < squaresPerSide; row++) {
            
            for ( int col = 0; col < squaresPerSide; col++) {
                
                int position = row * squaresPerSide + col;
                ...
                if( board[ position] == 0) {
                   ...
                   sprintf( name, "%d", board[ position]);   // "print" the square creation order number into a string to be stored in the square
                }
                // Set each array pointer have the address of a new Square, created with a Square constructor
                squaresArray[ position] = Square(squareSize, squareSize * col + col*10, (squareSize + 5) * row, sf::Color::White, true, name);
                // Draw the square
                window.draw( squaresArray[ position].getTheSquare());
                // Draw the text associated with the Square, in the window with the indicated color and text size
                int red = 0, green = 0, blue = 0;
                squaresArray[ position].displayText( &window, font, sf::Color(red, green, blue), 400 / (squaresPerSide * 2));
            }
        }
        
        
        // Display Graphical Move Number
        std::string name = "Move ";     
        char move[81];
        sprintf(move, "%d", pHead->moveData);
        name = name + move;
        
        squaresArray[ squaresPerSide * squaresPerSide] = Square(50, 35, 460, sf::Color::Black, true, name);
        squaresArray[ squaresPerSide * squaresPerSide].displayText( &window, font, sf::Color::White, 20);
        ...
        std::cout << pHead->moveData << ". Your move: ";
        std::cin >> userInput;
        switch (userInput) {
            case 'x':
            ...
            case 'u':
                    ...
                    if ( pHead->moveData <= 1) {
                        ...
                        // Reset Game Values
                        for (int i = 0; i < squaresPerSide*squaresPerSide; ++i) {
                            board[ i] = pHead->boardData[ i];
                        }
                        score = pHead->scoreData;
                    }
                    continue;
